//
//  ProfileEditIntroduceModal.swift
...
final class ProfileEditIntroduceModal: BaseController {
  // MARK: - View Property
  private lazy var mainView = ProfileEditIntroduceView()
  
  // MARK: - Reactor Property
  public typealias Reactor = ProfileEditIntroduceReactor
  ...
  deinit {
    print("해제됨: ProfileEditIntroduceModal")
  }
}

extension ProfileEditIntroduceModal: ReactorKit.View {
  public func bind(reactor: ProfileEditIntroduceReactor) {
    mainView.touchEventRelay
      .bind(with: self) { owner, event in
        ...
        case .inputText(let text):
          owner.reactor?.action.onNext(.inputText(text))
        case .change:
    ...
    reactor.state
      .map(\.inputedNicknameText)
      .distinctUntilChanged()
      .bind(with: self) { owner, text in
        owner.mainView.setTextCountLabel(text: text)
      }
      .disposed(by: disposeBag)
...
extension ProfileEditIntroduceModal {
